...
<Callout type="warn">
  ...
  it would create a duplicate compute budget instruction.
</Callout>
...
```ts
const result = await update({ id: streamId, amountPerPeriod: new BN(200) }, senderInvoker, env);
```

<Callout type="info">
  `canUpdateRate` is **not exposed** in `ICreateVestingParams`. Streams created via `createVesting` don't support
  `update` unless you use `SolanaStreamClient.create()` directly with `canUpdateRate: true`.
</Callout>
