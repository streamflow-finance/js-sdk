...
<Callout type="warn">
  If `cliffAmount >= amount - 1` and all lock flags are `false`, the stream silently reclassifies as a Lock on-chain with a different fee tier. Keep `cliffAmount` well below the total. See [Stream Classification](/docs/core-concepts/streams#stream-classification) for the full criteria.
</Callout>
