...
<Callout type="warn">
  `depositedAmount` is the on-chain value after protocol fees are added, so it is slightly **higher** than your `amount`. Keep `cliffAmount` meaningfully below `amount` to stay safe. There is no client-side warning - verify cliff amounts before creating production streams.
</Callout>
