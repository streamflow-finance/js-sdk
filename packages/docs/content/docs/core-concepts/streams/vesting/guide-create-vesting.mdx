...
<Callout type="info">
  The allocation stream uses internal parameters specifically chosen to avoid on-chain lock classification. These are
  set automatically - you only need to provide the `amount`.
</Callout>
