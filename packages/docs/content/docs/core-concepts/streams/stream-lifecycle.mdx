...
<Callout type="info">
  `canTopup` and `automaticWithdrawal` must be decided at creation - they cannot be changed afterwards. The cancel and
  transfer flags can be toggled later via `update()`, but only if the stream was originally created with `canUpdateRate:
  true`.
</Callout>
