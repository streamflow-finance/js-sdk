...
<Callout type="warn">
  The cliff condition is the easy one to accidentally trigger. A vesting stream with a very large `cliffAmount` (within
  1 of the total) will silently reclassify as a Lock if all flags are `false`, and be charged lock fee rates instead of
  vesting rates. Keep `cliffAmount` well below the total, or set at least one flag to `true`.
</Callout>
