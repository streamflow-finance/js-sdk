...
<Callout type="warn">
  `priceOracle` must be the specific oracle feed account for your token pair, not the oracle program ID. For Pyth, find the correct feed address at [pyth.network/price-feeds](https://pyth.network/price-feeds). For Switchboard, use [app.switchboard.xyz](https://app.switchboard.xyz).
</Callout>
