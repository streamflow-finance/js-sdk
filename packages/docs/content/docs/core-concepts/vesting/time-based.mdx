...
Providing `initialAllocation` creates two streams atomically and returns `BatchInstructionResult` instead of `CreateInstructionResult`. See [Guide: Create a Vesting Stream](/docs/core-concepts/vesting/guide-create-vesting) for the full pattern.
