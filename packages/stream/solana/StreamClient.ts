  ...
  async prepareCreateAlignedUnlockInstructions(
    ...
    const createIx = await this.alignedProxyProgram.methods
      .create({
        ...
        minPrice: typeof minPrice === 'number' ? getBN(minPrice, ALIGNED_PRECISION_FACTOR_POW) : minPrice ?? new BN(0),
        maxPrice: typeof maxPrice === 'number' ? getBN(maxPrice, ALIGNED_PRECISION_FACTOR_POW) : maxPrice ?? new BN(0),
        minPercentage: typeof minPercentage === 'number' ? getBN(minPercentage, ALIGNED_PRECISION_FACTOR_POW) : minPercentage ?? new BN(0),
        maxPercentage: typeof maxPercentage === 'number' ? getBN(maxPercentage, ALIGNED_PRECISION_FACTOR_POW) : maxPercentage ?? new BN(0),
